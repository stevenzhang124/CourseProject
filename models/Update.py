...
class LocalUpdate(object):
    def __init__(self, args, source, target_train, target_test, idxs=None):
        self.args = args
        self.source_loader = DataLoader(DatasetSplit(source, idxs), batch_size=self.args.local_bs, shuffle=True, num_workers=4)
        self.target_train_loader = DataLoader(DatasetSplit(target_train, idxs), batch_size=self.args.local_bs, shuffle=True, num_workers=4)
        self.target_test_loader = DataLoader(target_test, batch_size=self.args.bs, shuffle=True, num_workers=4)

    def train(self, model):
        optimizer = torch.optim.SGD([
            {'params': model.base_network.parameters()},
            {'params': model.bottleneck_layer.parameters(), 'lr': 10 * self.args.lr},
            {'params': model.classifier_layer.parameters(), 'lr': 10 * self.args.lr},
